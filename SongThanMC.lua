...
-- [ Giữ nguyên hàm UpdateWhitelist và các thiết lập GUI cơ bản ]
...
-- CẬP NHẬT TẠI ĐÂY: Update pet list để nhận cả Candy & Money
local function updatePetList()
    local pets = {}
    for _, tool in ipairs(LocalPlayer.Backpack:GetChildren()) do
        local lvl = tonumber(tool:GetAttribute("Level")) or 0
        local mutation = tool:GetAttribute("Mutation")
        if tool:IsA("Tool") and TARGET_MUTATIONS[mutation] and lvl >= MIN_LEVEL then
...
-- CẬP NHẬT TẠI ĐÂY: Logic Trade hỗ trợ Money
task.spawn(function()
    while true do
        task.wait(0.5)
        ...
        for _, tool in ipairs(LocalPlayer.Backpack:GetChildren()) do
            ...
            if tool:IsA("Tool") and TARGET_MUTATIONS[mutation] and lvl >= MIN_LEVEL then
                if not SelectedPetName or name == SelectedPetName then
                    ...
                    -- Giữ lại 1 con, chỉ gửi khi có từ 2 con trở lên
